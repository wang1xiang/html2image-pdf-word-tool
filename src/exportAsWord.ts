...
const handleCss = async () => {
  ...
  // eslint-disable-next-line
  const remoteCSSPromise = [...links].map((link: any) => fetch(link.href));
  const remoteCSSResult = await Promise.allSettled(remoteCSSPromise);
  // eslint-disable-next-line
  const remoteCSSStreamPromise = remoteCSSResult.map((item: any) => {
    const { status, value } = item;
  ...
  // eslint-disable-next-line
  const cssText = remoteCSSStreamResult.map((item: any) => {
    const { status, value } = item;
 ...
 * @returns Promise
 */
export function exportAsWord(selector: HTMLElement | string, filename: string) {
  const ele =
  ...
  return new Promise<void>(async (resolve) => {
    ...
    asBlob(htmlString, { margins }).then((data: Blob) => {
